...
The library manages three key tables:

1. **PRODUCT_READ_MODEL**
   - Stores search parameters and BaseProductData representation
   - Used for efficient product querying
   - Maintains synchronized copy of product data

2. **PRODUCT_EVENT**
   - Stores received ProductChangedEvents
   - Maintains event history for auditing
   - Tracks event processing status

3. **PRODUCT_STATUS**
   - Controls product update flow
   - Stores status flags that can suspend updates
   - Requires manual synchronization to resume suspended updates
   - Used for preventing automatic updates when manual intervention needed
